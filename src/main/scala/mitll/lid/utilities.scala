...
object utilities {
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------
  class Path(s: String) {
    def /(o: String) = if (s != "") s + File.separator + o else o

    def /(o: Any): String = this./(o.toString)

    def %(o: Any*) = s.format(o: _*)
  }
  ...
  implicit def toS[T](a: Arg[T]) = a.toString

  // Allow args to become strings
  def platform =
  ...
  def runProcess(command: String, addargs: String*): (Array[String], Int) = {
    ...
    val proc = p.start()
    val errs = Future {
      // prevent blocking on unread stderr
