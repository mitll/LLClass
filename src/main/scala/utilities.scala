...
package mitll
import java.io.{File, BufferedReader, BufferedWriter, BufferedOutputStream, InputStreamReader, InputStream, PrintStream, FileInputStream, FileOutputStream, IOException, PrintWriter, ObjectOutputStream, ObjectInputStream, BufferedInputStream}
...
import collection.mutable.{Map, HashMap, ArrayBuffer, ListBuffer}
...
object utilities {
  ...
  def runProcess(command : String, addargs : String*) : (Array[String], Int) = {
    ...
    proc.waitFor();
    source.close;
    return (ret.toArray, proc.exitValue());
  }
