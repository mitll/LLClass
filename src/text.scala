 ...
 * Copyright &copy; 2011 Massachusetts Institute of Technology, Lincoln Laboratory
 * @version 0.1a
 */
package mitll
import utilities._
...
class AP5 extends Function1[String, Array[String]] {
  ...
  def apply(in : String) : Array[String] = {
    ...
    return words.map(anorm(_)); //.mkString(" ").trim.replaceAll("""\s+""", " ");
  }
}
...
trait SimpleTokenizer extends Function1[String, Array[String]] {
  def recSplit(word : String, pat : Regex, postP : Boolean) : String = {
    ...
    return (if (pres.length > 0) pres.mkString(" ") + " " else "") + nw + (if (posts.length > 0) " " + posts.mkString(" ") else "");
  }
