...
package mitll
import utilities._
import java.io.{File, BufferedReader, BufferedWriter, BufferedOutputStream, InputStreamReader, InputStream, PrintStream, FileInputStream, FileOutputStream, IOException, PrintWriter, ObjectOutputStream, ObjectInputStream};
...
import collection.{Iterable, IterableView};
import collection.immutable.Vector;
import collection.mutable.{Map, HashMap, ArrayBuffer};
import org.apache.commons.io.FileUtils._;

//import tools.nsc.{Settings => ISettings, Properties, GenericRunnerSettings, Global, GenericRunnerCommand, ScriptRunner, ObjectRunner};
//import tools.nsc.util.{ScriptSourceFile};
//import tools.nsc.Properties.{versionString, copyrightString};
//import tools.nsc.GenericRunnerCommand._;
//import tools.nsc.interpreter.{ILoop, SimpleReader};
...
trait ProcessorRegistry extends ArgHandler with JobQFactory {
  ...
  def instantiateProcessor[T <: Processor](name : String, checkRegistry : Boolean = true) = {
    ...
    def getDefaults(cons : java.lang.reflect.Constructor[_], expected : Int) : Array[AnyRef] =  {
      ...
      if (numargs == expected) return Array[AnyRef]();
...
trait ExternalPipeSupport extends RegexParsers { self : ProcessorRegistry =>
  ...
  def stageToString(k : Int, inputs : List[PipeInput], p : Pi) : String = {
    ...
    return (0 until pipe.length).map { i => if (i == 0) pre + pipe(i) else " " * pre.length + pipe(i) }.mkString(" " + "->" + "\n");
  }
